...
const UpdateInventoryBalance = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        console.log("payload from update inventory balance", payload);
        await updateInventoryBalance(tenantId, id, payload);
        setNotification({
          open: true,
          type: "success",
          message: "Inventory Balance updated successfully!",
        });
        setTimeout(() => navigate("/list-inventory-balance"), 1500);
      } catch (error) {
        setNotification({
          open: true,
          type: "error",
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setInitialLoad(true);
        const [countsRes, itemsRes, balanceRes] = await Promise.all([
          getAllInventoryCounts(tenantId),
          getItems(tenantId),
          getInventoryBalanceById(tenantId, id),
        ]);

        setInventoryCounts(
          Array.isArray(countsRes.data.content) ? countsRes.data.content : []
        );
        setItems(Array.isArray(itemsRes.data) ? itemsRes.data : []);

        // Set form values from existing balance
        const balance = balanceRes.data;
        formik.setValues({
          preparedBy: balance.preparedBy || username,
          preparedOn: balance.preparedOn
            ? balance.preparedOn.split("T")[0]
            : new Date().toISOString().split("T")[0],
          inventoryCountId: balance.inventoryCountId || "",
          storeType: balance.storeType || STORE_TYPES.INTERNAL,
          inventoryBalanceItemRequest:
            balance.inventoryBalanceItemResponse?.map((item) => ({
              id: uuidv4(),
              itemId: item.itemId,
              quantity: item.quantity,
              binBalance: item.binBalance ?? 100,
              remark: item.remark ?? "",
            })) || [
              {
                id: uuidv4(),
  ...
  return (
    <Box p={isMobile ? 2 : 5}>
      <Header subtitle={`Update Inventory Balance - ${id}`} />
...
export default UpdateInventoryBalance;
